"""YAML helpers with a small fallback for the project's generated subset."""
...
try:  # PyYAML is the preferred production parser.
...
class SimpleYamlError(ValueError):
    """Raised when the fallback parser cannot understand the input."""
...
def strip_code_fence(text: str) -> str:
    ...
    match = re.match(r"^```(?:yaml|yml)?\s*(.*?)\s*```$", cleaned, flags=re.DOTALL | re.IGNORECASE)
...
def _load_simple_yaml(text: str) -> Any:
    ...
    if index != len(lines):
        raise SimpleYamlError(f"Unexpected YAML content near line {index + 1}.")
    return value
...
def _parse_block(lines: list[tuple[int, str]], index: int, indent: int) -> tuple[Any, int]:
    ...
    if current_indent < indent:
        return None, index
    if current_indent > indent:
        raise SimpleYamlError(f"Unexpected indentation near line {index + 1}.")
        ...
        if not remainder:
            ...
            if index < len(lines) and lines[index][0] > indent:
                ...
                if not isinstance(extra, dict):
                    raise SimpleYamlError(f"Expected mapping continuation near line {index + 1}.")
                item.update(extra)
        else:
...
def _parse_dict(lines: list[tuple[int, str]], index: int, indent: int) -> tuple[dict[str, Any], int]:
    ...
    while index < len(lines):
        ...
        if current_indent != indent or content.startswith("-"):
            break
        if ":" not in content:
            raise SimpleYamlError(f"Expected key/value pair near line {index + 1}.")
