...
| 能力 | 说明 |
| --- | --- |
| 聊天式工作台 | 左侧项目与历史记录，中间多轮对话输出，底部输入与上传 |
| 多轮改稿 | 已生成 YAML 后可继续输入“对白更口语化”“加强冲突”等修改要求 |
| 多源输入 | 支持手动粘贴、读取剪贴板、上传或拖拽 `.txt/.docx/.pdf` 文件 |
...
- 生成后可继续进行多轮修改，最新 YAML 会自动替换为改稿结果
